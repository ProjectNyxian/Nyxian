    ...
    override func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { [weak application] _ in
            ...
            let openMenu: UIMenuElement = UIAction(title: "Open", image: UIImage(systemName: "arrow.up.right.square.fill")) { _ in
                ...
                LDEProcessManager.shared().spawnProcess(withBundleIdentifier: application.bundleIdentifier, withItems: [:], withKernelSurfaceProcess: kernel_proc(), doRestartIfRunning: false)
            }
                    ...
                    if result,
                       LDEApplicationWorkspace.shared().installApplication(atBundlePath: bundle.bundleURL.path) {
                        DispatchQueue.main.async {
                            LDEProcessManager.shared().spawnProcess(withBundleIdentifier: bundle.bundleIdentifier, withItems: [:], withKernelSurfaceProcess: kernel_proc(), doRestartIfRunning: false)
                        }
                    } else {
