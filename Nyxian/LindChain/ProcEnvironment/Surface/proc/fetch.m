...
ksurface_error_t proc_ptr_for_pid(pid_t pid,
                                  ksurface_proc_t **proc,
                                  unsigned int *index)
{
    // Check to ensure its not a nullified surface or better said
    ...
    // Check if the caller holds the lock
    if(!reflock_is_locked_by_machthreadself(&(surface->reflock))) return kSurfaceErrorNotHoldingLock;
    
    // Setting proc to null
    *proc = NULL;
